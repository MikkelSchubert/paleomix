...
1. Initial steps
    ...
    Each reference sequences in FASTA format is indexed using `samtools faidx` and using the short-read aligner configured for the current project.
...
3. Mapping of reads
    ...
    3. The BAM is sorted using `samtools sort`, indexed using `samtools index`, and (optionally) validated using Picard `ValidateSamFile`.
...
4. Filtering of duplicates, recalculation (rescaling) of quality scores, and validation
    ...
    1. If enabled, PCR duplicates are filtered using Samtools `markdup` for SE and PE reads and using `paleomix rmdup_collapsed` for collapsed reads (see the :ref:`other_tools` section). PCR filtering is carried out per library.
    ...
    3. The resulting BAMs are indexed and (optionally) validated using Picard `ValidateSamFile`. Mapped reads at each position of the alignments are compared using the query name, sequence, and qualities. If a match is found, it is assumed to represent a duplication of input data (see :ref:`troubleshooting_bam`).
